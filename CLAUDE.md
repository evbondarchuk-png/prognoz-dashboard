...
- **`hof.js` (новый общий скрипт, подключён в index/mop/rop)** — кнопка «👥 Сравни себя с коллегами»
  в шапке Зала славы (патчит `window.openHof`, разметку кабинетов не трогали).
